...
class MapNavigationUtil {
    companion object {
        // 导航方式
        private const val NAVIGATION_AMAP = 0
        private const val NAVIGATION_BAIDU = 1
        private const val NAVIGATION_QQMAP = 2

        private const val VEHICLE_TYPE_ELEBIKE = "electric" // 电动车
        private const val VEHICLE_TYPE_CAR = "minivan" // 小汽车
            ...
            Thread {
                ...
                context.runOnUiThread {
                    if (!hasAmap && !hasBaidu && !hasQQ) {
                        ToastUtil.show(context, "您没有安装高德、百度或腾讯地图")
                    } else {
                        ...
                        rbAmap.setOnClickListener {
                            if (it is RadioButton && it.isChecked) {
                                //调起高德地图
                                navigation(context, endPointLat, endPointLng, endAddress, NAVIGATION_AMAP, vehicleType)
                                dialog.dismiss()
                            }
                        }
                        ...
                        rbBaidu.setOnClickListener {
                            if (it is RadioButton && it.isChecked) {
                                ...
                                //调起百度地图
                                navigation(context, lat, lng, endAddress, NAVIGATION_BAIDU, vehicleType)
                                dialog.dismiss()
                            }
                        }
                        ...
                        rbQQ.setOnClickListener {
                            if (it is RadioButton && it.isChecked) {
                                //调起腾讯地图
                                navigation(context, endPointLat, endPointLng, endAddress, NAVIGATION_QQMAP, vehicleType)
                                dialog.dismiss()
                            }
                        }
        ...
        private fun navigation(
            ...
            when (type) {
                NAVIGATION_AMAP -> {
                    ...
                    ToastUtil.show(context, "正在打开高德地图App")
                }
                NAVIGATION_BAIDU -> {
...
//                            "&mode=$mode")
                    ...
                    context.startActivity(intent)
                    ToastUtil.show(context, "正在打开百度地图App")
                }
                NAVIGATION_QQMAP -> {
                    ...
                    context.startActivity(intent)
                    ToastUtil.show(context, "正在打开腾讯地图App")
                }
            }
